...
| `AUTOTRADING_BUY_EARLY_PTB_GAP_BPS` | Optional. **Basis points** of (absolute spot minus price_to_beat) divided by price_to_beat. When `AUTOTRADING_BUY_LAST_SECS` is set, a BUY blocked only for being too early may still proceed if the gap meets this threshold (`0` = off). Never bypasses past-close |
...
Set **`AUTOTRADING_BUY_EARLY_PTB_GAP_BPS`** (basis points, default `0`) together with **`AUTOTRADING_BUY_LAST_SECS`** to allow an earlier autotrading BUY when spot has moved far enough from price-to-beat: the gate compares `abs(spot - price_to_beat) / price_to_beat` to `threshold / 10_000`. This applies only when the market is still open but outside the configured final window; it does **not** override a deny because the window has already ended.
...
Set **`AUTOTRADING_SIGNAL_MIN=watch`** to allow autotrading to enter on `WATCH` when the live spot direction is already clear. This is earlier and less selective than `STRONG`, so pair it with `AUTOTRADING_MAX_ENTRY_PRICE` if you want to avoid late/high-price entries. The default is `strong`.
